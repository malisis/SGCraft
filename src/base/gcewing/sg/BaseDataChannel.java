...
public class BaseDataChannel {
    ...
    @ServerMessageHandler(".container.") public void onServerContainerMessage(EntityPlayer player, ChannelInput data) {
        String message = data.readUTF();
        doServerDispatch(player.openContainer, message, player, data);
    }

    @SideOnly(Side.CLIENT) @ClientMessageHandler(".container.") public void onClientContainerMessage(ChannelInput data) {
        EntityPlayer player = net.minecraft.client.Minecraft.getMinecraft().player;
        String message = data.readUTF();
        doClientDispatch(player.openContainer, message, player, data);
    }

    protected void onReceiveFromClient(EntityPlayer player, ChannelInput data) {
        String message = data.readUTF();
        for (Object h : handlers)
            if (serverDispatch(h, message, player, data)) return;
    ...
    public static void doServerDispatch(Object handler, String message, EntityPlayer player, ChannelInput data) {
        if (!serverDispatch(handler, message, player, data)) System.out.printf("No ServerMessageHandler for '%s' found in %s\n", message, handler.getClass().getName());
    }
    ...
    protected void onReceiveFromServer(EntityPlayer player, ChannelInput data) {
        String message = data.readUTF();
        for (Object h : handlers)
            if (clientDispatch(h, message, player, data)) return;
        System.out.printf("No ClientMessageHandler for '%s' found in registered handlers\n", message);
    }

    public static void doClientDispatch(Object handler, String message, EntityPlayer player, ChannelInput data) {
        if (!clientDispatch(handler, message, player, data)) System.out.printf("No ClientMessageHandler for '%s' found in %s\n", message, handler.getClass().getName());
    }

    public static boolean clientDispatch(Object handler, String message, EntityPlayer player, ChannelInput data) {
        if (handler != null) {
        ...
        @Override public void channelRead(ChannelHandlerContext ctx, Object obj) throws Exception {
            if (obj instanceof FMLProxyPacket) {
                if (ctx.channel() == channel.pipes.get(Side.SERVER)) {
                    handleServerPacket(ctx, (FMLProxyPacket) obj);
                }
                if (ctx.channel() == channel.pipes.get(Side.CLIENT)) {
                    handleClientPacket(ctx, (FMLProxyPacket) obj);
                }
